...
__device__ Vector rk54bs_stage_1(double* Mn, double* H)
{
    Vector Data = EvaluateLandauLifshitz_GPU(Mn, H);

    return Data;
}
__device__ Vector rk54bs_stage_2(double* Mn1, double* Mn, double* H, double* k1)
{
    Vector Data;
    double a21 = 1. / 6.;
...
__device__ Vector rk54bs_stage_3(double* Mn1, double* Mn, double* H, double* k1, double* k2)
{
    Vector Data;
...
__device__ Vector rk54bs_stage_4(double* Mn1, double* Mn, double* H, double* k1, double* k2,
                                 double* k3)
{
    Vector Data;
...
__device__ Vector rk54bs_stage_5(double* Mn1, double* Mn, double* H, double* k1, double* k2,
                                 double* k3, double* k4)
{
    Vector Data;
...
__device__ Vector rk54bs_stage_6(double* Mn1, double* Mn, double* H, double* k1, double* k2,
...
__device__ Vector rk54bs_stage_7(double* Mn1, double* Mn, double* H, double* k1, double* k2,
...
__device__ Vector rk54bs_stage_8(double* Mn1, double* Mn, double* H, double* k1, double* k2,
...
__global__ void RungeKuttaStage_2_RK54BS(MAG M, FIELD H, MEMDATA DATA, int Flag)
{
...
#pragma unroll DIM
            ...
            Vector stage = rk54bs_stage_1(Mn, Heff);
            Vector k2 = rk54bs_stage_2(Mn1, Mn, Heff, &(stage.X[0]));
...
__global__ void RungeKuttaStage_3_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
...
#pragma unroll DIM
            ...
            Vector stage = rk54bs_stage_3(Mn1, Mn, Heff, k1, &(k2.X[0]));
...
__global__ void RungeKuttaStage_4_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
...
#pragma unroll DIM
            ...
            Vector stage = rk54bs_stage_4(Mn1, Mn, Heff, k1, k2, &(k3.X[0]));
...
__global__ void RungeKuttaStage_5_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
...
#pragma unroll DIM
            ...
            Vector stage = rk54bs_stage_5(Mn1, Mn, Heff, k1, k2, k3, &(k4.X[0]));
...
__global__ void RungeKuttaStage_6_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
...
#pragma unroll DIM
            ...
            Vector stage = rk54bs_stage_6(Mn1, Mn, Heff, k1, k2, k3, k4, &(k5.X[0]));
...
__global__ void RungeKuttaStage_7_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
...
#pragma unroll DIM
            ...
            Vector stage = rk54bs_stage_7(Mn1, Mn, Heff, k1, k2, k3, k4, k5, &(k6.X[0]));
...
__global__ void RungeKuttaStage_8_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    if (i < NUM && j < NUMY && k < NUMZ)
    {
        if (M->Mat[ind(i, j, k)] != 0)
        {
            double Mn1[3], Mn[3], Heff[3], k1[3], k2[3], k3[3], k4[3], k5[3], k6[3];
#pragma unroll DIM
            for (int n = 0; n < DIM; n++)
            {
                Mn[n] = M->M[mind(0, i, j, k, n)];
            ...
            Vector k7 = EvaluateLandauLifshitz_GPU(Mn1, Heff);
            Vector stage = rk54bs_stage_8(Mn1, Mn, Heff, k1, k2, k3, k4, k5, k6, &(k7.X[0]));
...
__global__ void RungeKuttaFinalSolution_RK54BS(MAG M, FIELD H, MEMDATA DATA)
{
    int i = threadIdx.z + blockIdx.z * blockDim.z;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.x + blockIdx.x * blockDim.x;
...
__host__ void   RungeKutta54BS_StageEvaluations(MAG M_d, FIELD H_d, MEMDATA DATA, PLANS P, double h, int* ResetFlag, int AllocSize)
{
    ...
    RungeKuttaStage_2_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator>> >
        (M_d, H_d, DATA, *ResetFlag);
    ...
    RungeKuttaStage_3_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_4_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_5_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_6_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_7_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_8_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaFinalSolution_RK54BS << <NumberofBlocksIntegrator, NumberofThreadsIntegrator>> >
        (M_d, H_d, DATA);
