...
#pragma unroll DIM
               ...
               // Flag = Crout_LU_with_Pivoting_Solve(&J_s.J[0][0], &F_0.X[0], Pivot_s, &dzi.X[0], DIM);
                Crout_LU_Solve(&J_s, &(dzi.X[0]), &F_0.X[0]);
...
#pragma unroll DIM
                    for (int i = 0; i < DIM; i++)
                    {
                       // z3[i] = Mn[i] + (1.230333209967908) * h_d * F_0.X[i];
                        z3[i] = Mk[i] + (1.230333209967908-0.520000000000000)*h_d*F_0.X[i];
                    }
                }
...
#pragma unroll DIM
                    ...
                    z4[i] = Mk[i] + (0.895765984350076- 1.230333209967908) * h_d * F_0.X[i];                        
                    }
                }
...
#pragma unroll DIM
            ...
            //Compute LU factorisation
           // LLJacobian_ESDIRK_GPU(&J_s, Mk, Heff, Da);
            LLJacobian_ESDIRK_GPU(&J_s, Mn, Heff, Da);
           // Flag = Crout_LU_Decomposition_with_Pivoting(&J_s.J[0][0], &(Pivot_s[0]), DIM);
            Crout_LU_Decomp(&J_s);
...
#pragma unroll DIM
                    for (int i = 0; i < DIM; i++)
                    {
                      //  z6[i] = Mn[i] + h_d * F_0.X[i];    
                        z6[i] = Mk[i] + (1.0 - 0.436393609858648)*h_d * F_0.X[i];
                    }
                }
...
__global__ void RungeKuttaFinalSolution_ESDIRK54a(MAG M, FIELD H, MEMDATA DATA)
{
    ...
    if (i < NUM && j < NUMY && k < NUMZ)
    {
        if (M->Mat[ind(i, j, k)] != 0)
        {
            ...
            double Mn[3], Mn1[3], Mnk[3], Heff[3], Mns[3];
...
#pragma unroll DIM
            ...
            Data.X[0] = 1.0 * sqrt(Mns[0] * Mns[0] + Mns[1] * Mns[1]
                + Mns[2] * Mns[2]);
    ...
    int* ResetFlag, int AllocSize)
{
    ...
    RungeKuttaStage_2_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator>> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_3_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_4_ESDIRK54a_TorqueAndField << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);

    checkCudaErrors(cudaPeekAtLastError());
    checkCudaErrors(cudaDeviceSynchronize());

    RungeKuttaStage_4_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_5_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_6_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaStage_7_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator >> >
        (M_d, H_d, DATA);
    ...
    RungeKuttaFinalSolution_ESDIRK54a << <NumberofBlocksIntegrator, NumberofThreadsIntegrator>> >
        (M_d, H_d, DATA);
